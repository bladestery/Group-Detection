...
{
    pair<typename range_const_iterator<R>::type, typename range_const_iterator<R>::type> p = std::minmax_element(boost::begin(r), boost::end(r));
...
{
	out << t.template get<0>() << " " << t.template get<1>() << " " << t.template get<2>() << " " << t.template get<3>();
	return out;
}
template<class T1, class T2, class T3, class T4, class T5> static ostream& operator << (ostream& out, const tuple<T1, T2, T3, T4, T5>& t)
{
	out << t.template get<0>() << " " << t.template get<1>() << " " << t.template get<2>() << " " << t.template get<3>() << " " << t.template get<4>();
	return out;
}
//PAIR OPERATIONS
